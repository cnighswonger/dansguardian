...
// when checkPICS is called we assume checkphrase has made the document
// lower case.
void NaughtyFilter::checkPICS(char* file, int l) {
    ...
    if (!(*o.fg[filtergroup]).pics1.matched()) {return;}  // exit if not found
...
void NaughtyFilter::checkPICSrating(std::string label) {
    ...
    if (!(*o.fg[filtergroup]).pics2.matched()) {return;}  // exit if not found
	...
	if (i == 0) {
        ...
        if (service.contains("safesurf")) {checkPICSratingSafeSurf(r);}
        if (isItNaughty) {return;}
        if (service.contains("evaluweb")) {checkPICSratingevaluWEB(r);}
        if (isItNaughty) {return;}
        if (service.contains("microsys")) {checkPICSratingCyberNOT(r);}
        if (isItNaughty) {return;}
        if (service.contains("icra")) {checkPICSratingICRA(r);}
        if (isItNaughty) {return;}
        if (service.contains("rsac")) {checkPICSratingRSAC(r);}
        if (isItNaughty) {return;}
        if (service.contains("weburbia")) {checkPICSratingWeburbia(r);}
        if (isItNaughty) {return;}
        if (service.contains("vancouver")) {checkPICSratingVancouver(r);}
...
// The next few funtions are flipin obvious so no explanation...
...
void NaughtyFilter::checkPICSagainstoption(String s, char* l, int opt, std::string m) {
    if (s.indexOf(l) != -1) {  // if the rating contains the label then:
        int i = 0;
        s = s.after(l);  // get the rating label value
        if (s.indexOf(" ") != -1) {s = s.before(" ");} //remove anything after it
        if (s.length() > 0) {  // sanity checking
