...
void ReadConfig(const char *configfile, int type) {
...
int main(int argc, char* argv[]) {
    ...
    for (int i=1; i<argc; i++) {
        ...
        switch (option) {
            ...
            case 'q':
                ...
                return sysv.kill(o.pid_filename);
            case 's':
                ReadConfig(configfile.c_str(), 0);
                return sysv.showPID(o.pid_filename);
            case 'r':
                ReadConfig(configfile.c_str(), 0);
                return sysv.hup(o.pid_filename);
            case 'g':
                ReadConfig(configfile.c_str(), 0);
                return sysv.usr1(o.pid_filename);
            case 'v':
            ...
            case 'h':
                ...
                if (option == 'h')
                    return 0;  // they asked for the help
                return 1;  // they need help
        }
    }
    ...
    ReadConfig(configfile.c_str(), 2);

    if (sysv.amIRunning(o.pid_filename)) {
    ...
    int pidfilefd = sysv.openPIDFile(o.pid_filename);  // we have to open/create
